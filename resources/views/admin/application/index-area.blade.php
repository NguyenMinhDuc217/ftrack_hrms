    ...
    <div class="col-sm-12">
      ...
      <div class="card table-responsive table-card mb-0">
        <table class="table table-hover mb-0" id="pc-dt-simple">
          ...
          @foreach ($applications_areas as $app_a)
          <tbody id="app_a_{{$app_a->application_id}}">
            <tr class="align-middle">
              <div>
                <td class="text-center">{{ $app_a->application_id }}</td>
                <td class="text-center text-truncate" style="max-width: 100px;">
                  ...
                  <a href="{{ $app_a->application->user_document->url }}" target="_blank" class="mb-0 fw-bold text-success text-decoration-underline bg-light p-2 rounded border-1 text-truncate">
                    {{ $app_a->application->user_document->document_title }}
                  </a>
                </td>
                <td>
                  <div class="row">
                    <div class="col lh-lg">
                      <p class="fw-bold mb-0 f-15">{{ $app_a->application->user->username }}</p>
                      <p class="text-muted f-12 mb-0"><i class="ti ti-mail"></i> {{ $app_a->application->user->email }}</p>
                      <p class="text-muted f-12 mb-0"><i class="ti ti-phone"></i> {{ $app_a->application->user->phone_number }}</p>
                    </div>
                  </div>
                </td>
                <td> 
                  <div class="col lh-lg">
                      <p class="fw-bold mb-0 f-15">{{ $app_a->application->job->name }}</p>
                      <p class="text-muted f-12 mb-0"><i class="ti ti-building-skyscraper"></i> {{ $app_a->application->job->organization->name }}</p>
                      @if($app_a->job_area->count() > 0)
                      <div class="text-sm text-gray-600">
                        <i class="ti ti-map-pin"></i>
                          <span class="px-2 py-1 bg-gray-100 rounded-md text-xs">
                              {{ $app_a->job_area->province->localized_name ?? '' }}
                          </span>
                      </div>
                      @endif
                  </div>
                ...
                <td class="text-center">
                  <ul class="list-inline me-auto mb-0">
                    <li class="list-inline-item align-bottom" data-bs-toggle="tooltip" title="Chi tiết">
                      <a href="{{ route('admin.applications.show', $app_a->application_id) }}" class="avtar avtar-xs btn-link-success">
